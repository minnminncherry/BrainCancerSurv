...
def main(args=None):
    input_dir = "/home/licongcong/Desktop/TCGA/tcga-gbm/genomic_data"
    output_dir = "/home/licongcong/Desktop/learning/medical_image_classification/TCGA/dataset/genomic_data"
    output_genomic_dir = "/var/lib/mysql-files/"
    json_file_path = os.path.join(input_dir, "metadata.cart.2026-04-07.json")

    gen_cls = GenomicDataCleaner(input_dir, output_dir)
    gen_cls.connect_db()

    # Connect to database
    if(args == 'INSERT_METADATA_JSON_DATA'):
        if gen_cls.conn:
    ...
    elif(args == 'INSERT_GENOMIC_TSV_DATA'):
        if gen_cls.conn:
            first = True
    ...
    elif(args == 'GENERATE_CLEAN_GENOMIC_DATA'):
        if gen_cls.conn:
