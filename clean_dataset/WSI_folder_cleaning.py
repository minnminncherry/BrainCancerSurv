...
class WSIDataCleaner:
    ...
    def WSI_format_change_pt(self, output_file_path: str | Path, input_path: str | Path, final_metadata_file_path: str | Path):
        ...
        final_metadata_file_path = Path(final_metadata_file_path)
        ...
        for root, dirs, files in os.walk(input_path):
            for file in files:
                if file.endswith(valid_extensions):
                    ...
                    data.append({
                        'case_id': file.rsplit('.', 1)[0],
                        'pt_file_path': str(pt_path)
                    })
        # Save metadata CSV linking case IDs to .pt file paths
        metadata_df = pd.DataFrame(data)    
        metadata_csv_path = final_metadata_file_path
        metadata_df.to_csv(metadata_csv_path, index=False)
        print(f"Saved WSI metadata CSV to {metadata_csv_path}")
    ...
    def insert_wsi_json_data_to_db(self, json_data, table_name):
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                # Insert data
                sql = f"INSERT INTO {table_name} (submitter_id, entity_submitter_id, file_name, case_id, entity_id, experimental_strategy) VALUES (%s, %s, %s, %s, %s, %s)"
                
                values = [(item['submitter_id'], item['entity_submitter_id'], 
                          item['file_name'], item['case_id'], item['entity_id'], item['experimental_strategy']) 
                         for item in json_data]
